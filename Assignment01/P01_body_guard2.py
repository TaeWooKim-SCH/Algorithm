...
def solution(test_case):
    ...
    ######### 입력처리 ###########
    split_input: list[str] = test_case.split('\n'); # 인풋을 줄바꿈 기준으로 분리
    N, M = map(int, split_input[0].split(' ')); # 철수의 집 크기 (N, M)
    board: list[str] = split_input[1:]; # 현재 배치된 보디가드의 위치를 나타내는 행렬
    ...
    is_exist_rows: list[bool] = [False] * N;
    is_exist_cols: list[bool] = [False] * M;
    
    for row_i in range(N):
        if (any(board[row_i])): # or 연산
            is_exist_rows[row_i] = True;
    for col_i in range(M):
        for row in board:
            if (row[col_i]):
                is_exist_cols[col_i] = True;
                break; # 더 이상 탐색할 필요 없음. 해당 열은 이미 보디가드가 존재하기 떄문

    result_rows = is_exist_rows.count(False);
    result_cols = is_exist_cols.count(False);

    result = max(result_rows, result_cols);
    #####################################################
